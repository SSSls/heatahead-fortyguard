...
function currentLocalHour(timezone: string) {
  const parts = Object.fromEntries(new Intl.DateTimeFormat("en-CA", { timeZone: timezone, year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", hourCycle: "h23" }).formatToParts(new Date()).map((part) => [part.type, part.value]));
  return `${parts.year}-${parts.month}-${parts.day}T${parts.hour}:00`;
}

function isProcessing(status: string) {
  return status === "processing_spatial" || status === "processing_environment";
}
...
const initialForm: FormState = {
  ...
  analysisTimeLocal: currentLocalHour("America/New_York"),
  facilityRadiusM: "360",
  itLoadMw: "100",
  itLoadFraction: "0.80",
  baselinePue: "",
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("/api/analyses", { cache: "no-store" })
      ...
      .then((payload) => { if (active) setHistory(payload.analyses); })
  ...
  function choosePreset(id: string) {
    ...
    setForm((current) => ({ ...current, facilityName: selected.name, latitude: selected.lat, longitude: selected.lon, ianaTimezone: selected.timezone, analysisTimeLocal: currentLocalHour(selected.timezone) }));
  }
  ...
  return (
    <main>
      ...
      <section className="hero compact-hero">
        <div className="hero-copy"><p className="eyebrow">Proactive thermal decision support</p><h1>See heat risk<br />before the alarm.</h1><p className="lede">HeatAhead combines live or historical FortyGuard exposure with a locked ESIF cooling model, giving operators a location-aware planning window before facility telemetry reaches a protective threshold.</p><div className="hero-actions"><a className="primary-link" href="#analyze">Analyze a facility ↓</a><a href="#evidence">Inspect the evidence</a></div></div>
        <div className="method-stamp"><span>Evidence layers</span><strong>Exposure ≠ Cooling impact</strong><p>Spatial heat is reported directly. Only point weather enters the trained ESIF model.</p></div>
      </section>
      ...
      <section className="workspace-grid" id="analyze">
        <form className="analysis-form" onSubmit={submit}>
          ...
          <label className="field"><span>IANA timezone</span><select value={form.ianaTimezone} onChange={(event) => setForm({ ...form, ianaTimezone: event.target.value, analysisTimeLocal: currentLocalHour(event.target.value) })}>{timezones.map((timezone) => <option key={timezone}>{timezone}</option>)}</select></label>
          ...
          <div className="time-alignment full"><strong>Timezone-safe</strong><span>The local hour is converted to UTC on the server before the FortyGuard request; both timestamps are saved with the result.</span></div>
          <div className="form-section-label full"><strong>Operating scenario</strong><span>Only point weather and load state enter the ESIF model. Spatial layers stay separate.</span></div>
          <label className="field"><span>Core square width</span><div className="input-unit"><input type="number" min="100" max="1000" value={form.facilityRadiusM} onChange={(event) => setForm({ ...form, facilityRadiusM: event.target.value })} /><i>m</i></div><small>Centered square AOI—not a radius or an inferred campus boundary.</small></label>
          <label className="field"><span>Cooling configuration</span><select value={form.coolingConfiguration} onChange={(event) => setForm({ ...form, coolingConfiguration: event.target.value })}><option value="unknown">Unknown</option><option value="air-cooled">Air cooled</option><option value="evaporative">Evaporative</option><option value="chilled-water">Chilled water</option><option value="direct-to-chip">Direct-to-chip liquid</option><option value="hybrid">Hybrid</option></select></label>
          <label className="field"><span>IT load · optional</span><div className="input-unit"><input type="number" min="0" max="2000" step="1" value={form.itLoadMw} onChange={(event) => setForm({ ...form, itLoadMw: event.target.value })} placeholder="Unknown" /><i>MW</i></div></label>
          <div className="field full load-state-control"><div className="load-state-heading"><span>Load state · 45–105% in 5% steps</span><label><span>Exact value</span><select aria-label="Exact load state" value={form.itLoadFraction} onChange={(event) => setForm({ ...form, itLoadFraction: event.target.value })}>{LOAD_STATE_OPTIONS.map((option) => <option key={option.percent} value={option.fraction.toFixed(2)}>{option.percent}%</option>)}</select></label><strong>{selectedLoadState.percent}%</strong></div><input aria-label="Load state slider" type="range" min="0.45" max="1.05" step="0.05" value={form.itLoadFraction} onChange={(event) => setForm({ ...form, itLoadFraction: Number(event.target.value).toFixed(2) })} /><div className="load-ticks" aria-hidden="true">{LOAD_STATE_OPTIONS.map((option) => <i key={option.percent} />)}</div><div className="range-labels"><i>45%</i><i>60%</i><i>75%</i><i>90%</i><i>105%</i></div><p className="load-recommendation"><strong>Best-supported choices</strong><span>60%, 65%, 70%, 75%, 85%, 90%, 95%, or 100%. The 70% state has the most ESIF training coverage.</span></p><p className={`load-support support-${selectedLoadState.support}`}><strong>{loadSupportLabel(selectedLoadState)}</strong><span>{selectedLoadState.support === "sparse" ? "Limited ESIF coverage; overall transfer confidence will be Low." : "Hours within ±2.5 percentage points in the ESIF training block."}</span></p></div>
          <label className="field full"><span>Baseline PUE · optional</span><input type="number" min="1" max="2.5" step="0.001" value={form.baselinePue} onChange={(event) => setForm({ ...form, baselinePue: event.target.value })} placeholder="Leave blank to hide absolute scenario PUE" /></label>
          ...
          <div className="consent full">
            <div className="consent-row"><input id="save-history" type="checkbox" checked={form.saveForHistory} onChange={(event) => setForm({ ...form, saveForHistory: event.target.checked })} /><label htmlFor="save-history"><strong>Save this run to my facility history</strong><small>Stores inputs, summarized API data, model version, and outputs.</small></label></div>
          ...
          <button className="run-button full" disabled={loading}>{loading ? <><span className="spinner" /> Analysis running · {elapsedSeconds}s</> : "Analyze facility"}</button>
          <p className="form-footnote full">One run requests three centered square AOIs: selected core width at 60 m granularity; 1.2 km × 1.2 km neighborhood at 80 m; and 2.4 km × 2.4 km context at 100 m. These are HeatAhead choices submitted to the API, not boundaries returned by FortyGuard.</p>
          <details className="explain-panel full"><summary>What every input changes</summary><div className="explain-grid"><article><strong>Facility name</strong><p>Record label only. It never enters the API or model.</p></article><article><strong>Latitude / longitude</strong><p>Required API location and the center of all three square AOIs. A valid U.S. bounding-box value is not a guarantee of usable tiles; the completed API run is the coverage test.</p></article><article><strong>Timezone + local time</strong><p>Converted server-side to UTC and checked against the API timestamp. A mismatch lowers Transfer Confidence.</p></article><article><strong>Core square width</strong><p>Sets only the facility-core heatmap, 100–1,000 m wide. It does not change the ESIF model.</p></article><article><strong>Cooling configuration</strong><p>Applicability metadata. Unknown lowers confidence; the current model does not learn configuration-specific coefficients.</p></article><article><strong>IT load (MW)</strong><p>Scaling input: cooling ratio × MW. It is important for absolute MW, but it is not fed into the trained model and does not pretend ESIF was trained at hyperscale MW.</p></article><article><strong>Load state (%)</strong><p>Actual IT load divided by ESIF train-only P95 (≈3.66 MW). This normalized value enters the model; support hours show how much comparable training evidence exists.</p></article><article><strong>Baseline PUE</strong><p>Optional same-facility scenario anchor. Output is baseline + modeled weather uplift; it is not a trained absolute-PUE prediction.</p></article><article><strong>Save / improvement consent</strong><p>Save controls this browser/customer timeline. Shared improvement is separate, off by default, and unused in this Demo.</p></article></div></details>
        </form>
      ...
      <section className="evidence-section" id="evidence">
        ...
        <div className="evidence-grid">
          ...
          <article><div><span>Step 2</span><b className="status-partial">PARTIAL</b></div><h3>Frontier calibration</h3><strong>R² 0.828 · 50.8% MAE gain</strong><p>Operational baseline transfer passed. The cross-facility weather coefficient was rejected and is not used in customer claims.</p></article>
        ...
        <p className="claim-boundary"><strong>Claim boundary:</strong> Environmental Exposure is direct API analysis. Predicted Cooling Impact is ESIF-equivalent. Scenario PUE is baseline PUE plus modeled uplift—not a trained absolute-PUE model.</p>
      </section>
      ...
      <section className="history-section" id="history">
        ...
        {historyLoading ? <p className="history-empty">Loading private history…</p> : history.length === 0 ? <p className="history-empty">No saved analyses yet. Keep “Save this run” checked to start a facility timeline.</p> : <div className="history-layout"><div className="timeline" aria-label="Saved exposure timeline">{completedHistory.slice().reverse().map((item) => <button key={item.id} title={`${item.facilityName}: ${format(item.environmentalExposureScore, 0)}/100`} onClick={() => setAnalysis(item)}><span style={{ height: `${Math.max(8, ((item.environmentalExposureScore ?? 0) / historyMax) * 100)}%` }} /></button>)}</div><div className="history-list">{history.map((item) => <article className="history-row" key={item.id}><button className="history-open" onClick={() => setAnalysis(item)}><span><strong>{item.facilityName}</strong><small>{item.analysisTimeLocal.replace("T", " ")} · {item.ianaTimezone.replace("America/", "")}</small></span><span><strong className={isProcessing(item.status) ? "level-processing" : levelClass(item.environmentalExposureLevel)}>{item.status === "completed" ? item.environmentalExposureLevel : isProcessing(item.status) ? "Processing" : "Failed"}</strong><small>{item.status === "completed" ? `${format(item.wetBulbTemperatureC, 1)}°C wet bulb · ${Math.round(item.itLoadFraction * 100)}% load` : item.summary}</small></span><b>{isProcessing(item.status) ? "Resume →" : "View →"}</b></button><button type="button" className="history-delete" disabled={deletingId === item.id} onClick={() => deleteAnalysis(item)} aria-label={`Permanently delete ${item.facilityName}`}>{deletingId === item.id ? "Deleting…" : "Delete"}</button></article>)}</div></div>}
        <p className="history-policy">Delete permanently removes this browser/customer-scoped D1 record and cannot be undone. It does not cancel a FortyGuard activity that was already submitted. Hide/restore and retention rules remain future work.</p>
      </section>
      ...
      <section className="method">
        ...
        <ol><li><span>01</span><div><strong>Environmental Exposure</strong><p>Direct FortyGuard spatial statistics across core, neighborhood, and local background.</p></div></li><li><span>02</span><div><strong>Predicted Cooling Impact</strong><p>Locked ESIF point-weather model. Spatial heatmap values are not inserted into training.</p></div></li><li><span>03</span><div><strong>Transfer Confidence</strong><p>Never above Medium until API-to-ESIF and cross-facility weather transfer are externally validated.</p></div></li></ol>
      </section>

      <footer><p><strong>Important:</strong> Cooling outputs are ESIF-equivalent scenarios, not measured customer-facility PUE or cooling telemetry.</p><span>Demo v2.2 · verified pipeline · D1 history · API key server-side</span></footer>
    </main>
  );
}
...
function Result({ analysis }: { analysis: Analysis }) {
  ...
  return <>
    ...
    <div className="result-cards"><article><p>Environmental exposure</p><strong className={levelClass(analysis.environmentalExposureLevel)}>{analysis.environmentalExposureLevel}</strong><span>{format(analysis.environmentalExposureScore, 0)} / 100 heuristic index</span></article><article><p>Predicted cooling impact</p><strong>{analysis.incrementalCoolingMw === null ? signed(analysis.weatherUplift, 4) : `${signed(analysis.incrementalCoolingMw, 2)} MW`}</strong><span>{analysis.itLoadMw === null ? "weather uplift per unit IT" : `at ${format(analysis.itLoadMw, 0)} MW IT load`}</span></article><article><p>Transfer confidence</p><strong className="confidence">{analysis.confidenceLevel}</strong><span>Applicability label—not a prediction interval</span></article></div>
    ...
    <div className="impact-grid"><div><span>Point weather used by model</span><strong>{format(analysis.coreMeanTemperatureC, 1)}°C · {format(analysis.relativeHumidityPercent, 0)}% RH</strong><small>{format(analysis.wetBulbTemperatureC, 1)}°C wet bulb</small></div><div><span>ESIF-equivalent cooling</span><strong>{analysis.coolingMw === null ? `${format(analysis.coolingRatio, 4)} per IT` : `${format(analysis.coolingMw, 2)} MW`}</strong><small>ratio × customer IT MW; locked model {analysis.modelId}</small></div><div><span>Load-state evidence</span><strong>{Math.round(analysis.itLoadFraction * 100)}%</strong><small>{support ? loadSupportLabel(support) : `${analysis.loadStateSupport ?? "legacy"} record`}</small></div><div><span>Scenario PUE</span><strong>{analysis.scenarioPue === null ? "Not shown" : format(analysis.scenarioPue, 4)}</strong><small>{analysis.scenarioPue === null ? "enter same-facility baseline PUE to enable" : `${format(analysis.baselinePue, 3)} baseline + ${signed(analysis.weatherUplift, 4)} uplift`}</small></div></div>
    <details className="result-explainer"><summary>How to read every output and its trust level</summary><div className="explain-grid"><article><strong>Exposure score / level</strong><p>A 0–100 heuristic combining core p90 dry-bulb, wet bulb, core/background delta and hotspot fraction. Direct API-derived analysis, but not a probability of failure.</p></article><article><strong>Cooling ratio / MW</strong><p>The trained ESIF-equivalent cooling-to-IT ratio. If IT MW is supplied, proportional scaling creates MW; the target facility itself was not used for training.</p></article><article><strong>Weather uplift</strong><p>Model output minus the same hour/day/load scenario at 18°C and 50% RH. This isolates the modeled weather increment.</p></article><article><strong>Scenario PUE</strong><p>User baseline PUE + weather uplift. ESIF contains measured PUE, but the direct absolute-PUE model failed its gate, so this is deliberately an anchored scenario.</p></article><article><strong>Transfer Confidence</strong><p>Low if time is misaligned, cooling type is unknown, or load support is sparse; otherwise Medium. It never reaches High because cross-facility weather transfer is not validated.</p></article><article><strong>Spatial statistics</strong><p>Mean describes the AOI average; p90 is exceeded by about 10% of core tiles; max is the hottest tile; σ is within-core spread; core delta compares core mean with 2.4 km context.</p></article></div></details>
    <div className="result-meta"><span>Local: {analysis.analysisTimeLocal.replace("T", " ")}</span><span>UTC: {analysis.analysisTimeUtc.replace("T", " ").slice(0, 16)}</span><span>{analysis.latitude.toFixed(4)}, {analysis.longitude.toFixed(4)}</span><span>API: {analysis.apiTimestamp?.replace("T", " ").slice(0, 16) ?? "timestamp unavailable"}</span></div>
  </>;
}

function ProcessingResult({ analysis, onResume }: { analysis: Analysis; onResume: () => void }) {
  const stage = analysis.status === "processing_environment" ? "Humidity and wet-bulb analysis" : "Spatial temperature layers";
...
function AnalysisProgress({ seconds, status }: { seconds: number; status?: string }) {
  ...
  const activeStage = status === "processing_environment" ? 2 : status === "processing_spatial" ? 1 : 0;
  const progress = activeStage === 2 ? Math.min(92, 68 + seconds * 0.35) : activeStage === 1 ? Math.min(65, 25 + seconds * 0.55) : 12;
  return <div className="progress-state"><span className="progress-orbit"><i /><i /><i /></span><p className="eyebrow">02 · Live analysis</p><h2>{stages[activeStage]}</h2><p>FortyGuard heatmaps are asynchronous and may take more than a minute. HeatAhead is checking the same saved activities instead of restarting the run.</p><div className="progress-track" aria-label={`Analysis progress ${Math.round(progress)} percent`}><span style={{ width: `${progress}%` }} /></div><ol>{stages.map((stage, index) => <li className={index < activeStage ? "complete" : index === activeStage ? "active" : ""} key={stage}><b>{index < activeStage ? "✓" : index + 1}</b><span>{stage}</span></li>)}</ol><small>{seconds}s elapsed · activity IDs are stored server-side; a saved run can be resumed from History after a refresh</small></div>;
}
