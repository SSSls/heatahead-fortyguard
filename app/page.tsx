    ...
    <main>
      ...
      <section className="workspace-grid" id="analyze">
        <form className="analysis-form" onSubmit={submit}>
          ...
          <label className="field"><span>Facility footprint width</span><div className="input-unit"><input type="number" min="100" max="1000" value={form.facilityRadiusM} onChange={(event) => setForm({ ...form, facilityRadiusM: event.target.value })} /><i>m</i></div></label>
          ...
          <p className="form-footnote full">One run requests the selected core footprint / 60 m, nearby 1.2 km / 80 m, and background 2.4 km / 100 m heatmaps. Historical dates start in 2019; forecast dates are limited to 12 hours.</p>
        </form>

        <section className={`result-panel ${analysis || loading ? "has-result" : "empty-result"}`} id="analysis-result" aria-live="polite">
          {loading ? <AnalysisProgress seconds={elapsedSeconds} status={analysis?.status} /> : !analysis ? <div className="empty-state"><span>02</span><h2>Your result appears here</h2><p>Location alone produces Environmental Exposure. Add IT load for MW impact. Add baseline PUE only when you want a scenario PUE.</p><div className="layer-preview"><i>Core</i><i>≈1 km</i><i>≈2–3 km</i></div></div> : isProcessing(analysis.status) ? <ProcessingResult analysis={analysis} onResume={() => resumeAnalysis(analysis)} /> : analysis.status !== "completed" ? <div className="empty-state"><span>!</span><h2>Run failed</h2><p>{analysis.summary}</p></div> : <Result analysis={analysis} />}
        </section>
      </section>
      ...
      <section className="evidence-section" id="evidence">
        ...
        <div className="evidence-grid">
          <article><div><span>Step 1</span><b className="status-pass">PASS</b></div><h3>ESIF internal model</h3><strong>R² 0.448 · MAE 0.003246</strong><p>Chronological test set; cooling-ratio target passed the calibration gate and temperature monotonicity check.</p></article>
      ...
      <section className="history-section" id="history">
        ...
        {historyLoading ? <p className="history-empty">Loading private history…</p> : history.length === 0 ? <p className="history-empty">No saved analyses yet. Keep “Save this run” checked to start a facility timeline.</p> : <div className="history-layout"><div className="timeline" aria-label="Saved exposure timeline">{completedHistory.slice().reverse().map((item) => <button key={item.id} title={`${item.facilityName}: ${format(item.environmentalExposureScore, 0)}/100`} onClick={() => setAnalysis(item)}><span style={{ height: `${Math.max(8, ((item.environmentalExposureScore ?? 0) / historyMax) * 100)}%` }} /></button>)}</div><div className="history-list">{history.map((item) => <button className="history-row" key={item.id} onClick={() => setAnalysis(item)}><div><strong>{item.facilityName}</strong><span>{item.analysisTimeLocal.replace("T", " ")} · {item.ianaTimezone.replace("America/", "")}</span></div><div><strong className={isProcessing(item.status) ? "level-processing" : levelClass(item.environmentalExposureLevel)}>{item.status === "completed" ? item.environmentalExposureLevel : isProcessing(item.status) ? "Processing" : "Failed"}</strong><span>{item.status === "completed" ? `${format(item.wetBulbTemperatureC, 1)}°C wet bulb · ${Math.round(item.itLoadFraction * 100)}% load` : item.summary}</span></div><b>{isProcessing(item.status) ? "Resume →" : "View →"}</b></button>)}</div></div>}
        <p className="history-policy">This iteration supports saving and viewing. Hide, restore, permanent deletion, retention rules, and automated hourly refresh remain the next data-lifecycle iteration.</p>
      </section>
...
function Result({ analysis }: { analysis: Analysis }) {
  ...
  return <><div className="result-heading"><div><p className="eyebrow">02 · Analysis result</p><h2>{analysis.facilityName}</h2><p>{analysis.summary}</p></div><span className="saved-badge">{analysis.saveForHistory === false ? "Live scenario" : "Saved record"}</span></div><div className="operator-action"><span>Suggested operator review</span><strong>{operatorAction(analysis)}</strong><small>Decision support only—confirm against facility controls, redundancy, and operating procedures.</small></div><div className="result-cards"><article><p>Environmental exposure</p><strong className={levelClass(analysis.environmentalExposureLevel)}>{analysis.environmentalExposureLevel}</strong><span>{format(analysis.environmentalExposureScore, 0)} / 100 heuristic index</span></article><article><p>Predicted cooling impact</p><strong>{analysis.incrementalCoolingMw === null ? signed(analysis.weatherUplift, 4) : `${signed(analysis.incrementalCoolingMw, 2)} MW`}</strong><span>{analysis.itLoadMw === null ? "weather uplift per unit IT" : `at ${format(analysis.itLoadMw, 0)} MW IT load`}</span></article><article><p>Transfer confidence</p><strong className="confidence">{analysis.confidenceLevel}</strong><span>ESIF internal validation; no Frontier weather transfer</span></article></div><div className="spatial-block"><div className="subheading"><strong>Spatial environment</strong><span>modeled outdoor temperature</span></div><div className="spatial-grid"><article><span>Facility core</span><strong>{format(analysis.coreMeanTemperatureC, 1)}°C</strong><small>p90 {format(analysis.coreP90TemperatureC, 1)}° · max {format(analysis.coreMaxTemperatureC, 1)}°</small></article><article><span>≈1 km neighborhood</span><strong>{format(analysis.nearMeanTemperatureC, 1)}°C</strong><small>captures nearby roads and industrial surface</small></article><article><span>≈2–3 km background</span><strong>{format(analysis.backgroundMeanTemperatureC, 1)}°C</strong><small>core delta {signed(analysis.coreMinusBackgroundC, 1)}°C</small></article></div></div><div className="impact-grid"><div><span>Point weather used by model</span><strong>{format(analysis.coreMeanTemperatureC, 1)}°C · {format(analysis.relativeHumidityPercent, 0)}% RH</strong><small>{format(analysis.wetBulbTemperatureC, 1)}°C wet bulb</small></div><div><span>ESIF-equivalent cooling</span><strong>{analysis.coolingMw === null ? `${format(analysis.coolingRatio, 4)} per IT` : `${format(analysis.coolingMw, 2)} MW`}</strong><small>locked model {analysis.modelId}</small></div><div><span>Load-state evidence</span><strong>{Math.round(analysis.itLoadFraction * 100)}%</strong><small>{support ? loadSupportLabel(support) : `${analysis.loadStateSupport ?? "legacy"} record`}</small></div><div><span>Scenario PUE</span><strong>{analysis.scenarioPue === null ? "Not shown" : format(analysis.scenarioPue, 4)}</strong><small>{analysis.scenarioPue === null ? "enter baseline PUE to enable" : `${format(analysis.baselinePue, 3)} baseline + weather uplift`}</small></div></div><div className="result-meta"><span>Local: {analysis.analysisTimeLocal.replace("T", " ")}</span><span>UTC: {analysis.analysisTimeUtc.replace("T", " ").slice(0, 16)}</span><span>{analysis.latitude.toFixed(4)}, {analysis.longitude.toFixed(4)}</span></div></>;
}
